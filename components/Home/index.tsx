  ...
  return (
    <div className="flex-1 py-10 px-7 ">
      ...
      <div className="bg-white py-2 px-4 w-[40%] rounded-full flex items-center gap-2">
        ...
        <input
          className="focus-within:outline-none flex-1"
          type="text"
          onChange={(e) => {
            if (e.target.value)
              setPosts(
                postsRef.current.filter((d) =>
                  d.node.name
                    .toLowerCase()
      ...
      <div className="mt-5">
        <TopNftCard
          values={[
            {
              userImage: `https://dl.openseauserdata.com/cache/originImage/files/${
                posts[6]?.node?.banner?.split("files/")?.[1]?.split("?")?.[0]
              }`,
              coinImage:
